...
for i=1:n
    ...
    switch l.type
        ...
        case 'fc_custom'
            res(i+1).x = reshape(res(i).x,1,1,256,[]); 
        case 'custom'
            im = imresize(x,0.5);
        ...
        case 'custom48'
            im24 = imresize(x,0.5);
            im24 = im24 + net48.imageMean - net24.imageMean;
            im12 = imresize(x,0.25);
            im12 = im12 + net48.imageMean - net12.imageMean;
...
if doder
    ...
    for i=n:-1:max(1, n-opts.backPropDepth+1)
        ...
        switch l.type
            ...
            case 'fc_custom'
                res(i).dzdx = reshape(res(i+1).dzdx,16,16,1,[]); 
            case 'custom'
                wl1=gpuArray(l.weights{1});
                wl2=gpuArray(l.weights{2});
                [res(i).dzdx, res(i).dzdw{1}, res(i).dzdw{2}] = ...
